...
class PostsPagesTests(TestCase):
    ...
    def test_pages_uses_correct_template(self):
        template_pages_names = {
            ...
            reverse('posts:group_list', args=tuple[PostsPagesTests.group.slug]):
                'posts/group_list.html',
            reverse('posts:profile',
                    args=[PostsPagesTests.user.username]):
                'posts/profile.html',
            reverse('posts:post_detail', args=tuple[PostsPagesTests.post.id]):
                'posts/post_deatail.html',
            reverse('posts:post_create'): 'posts/create_post.html',
            reverse('posts:post_edit', args=tuple[PostsPagesTests.post.id]):
    ...
    def test_create_post_page_correct_context(self):
        ...
        for value, expected in self.form_fields_post_create.items():
            with self.subTest(value=value):
                ...
                self.assertIsInstanse(form_field, expected)
